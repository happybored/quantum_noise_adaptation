...
def main(train_noise_model,test_noise_model,backend,timestamp,qubit_noise =None):
...
#set noise model
    ...
    if train_flag:
        ...
        print("train cost: {:.2f}s".format(training_time))
        ...
        if admm_args.admm:
            ...
            scheduler = CosineAnnealingLR(optimizer, T_max=admm_args.epochs)
            ...
            print("admm cost: {:.2f}s".format(admm_time))
        ...
        if admm_args.masked_retrain:
            ...
            optimizer = torch.optim.Adam(model.parameters(), admm_args.lr)
            scheduler = None
            scheduler = CosineAnnealingLR(optimizer, T_max=admm_args.tuning_epochs)
            model.to(device)
            ...
            best_metrics1 =[0]
            start_time = time.time()
            for epoch in range(1, admm_args.tuning_epochs + 1):
            ...
            print("turning cost: {:.2f}s".format(tuning_time))
            compression_time = tuning_time +admm_time
            print("compression cost: {:.2f}s".format(compression_time))


        model = copy.deepcopy(best_model)
        # torch.save(best_model,'dynamic_model/{}'.format(args.modelpath))
    else:
        pass
    ...
    torch.save(best_model,'dynamic_model_iris_jakarta/{}/date{}_pr{}_acc{}'.format(args.pruning_type,timestamp,admm_args.prune_ratio,acc))
    result =  dict()
...
if __name__ == '__main__':
    ...
    sys.stdout = Logger('./logger/{}_log.txt'.format('noise_awaregrid_search'))
    import datetime    
    ##load csv 
    from qiskit import IBMQ
    ...
    fname = 'calibration_data/prune_records_jakarta.xlsx'  
    # df = pd.read_csv(path)
    df = pd.read_excel(path)
    df = pd.DataFrame(df)

    df2 = df.copy(deep=True)
    # df2['accuracy-prune-test'] = 0
    ...
    for i in range(0,len(df2)):
        ...
        # df2['accuracy-prune-test'].loc[i] = result['accuracy'][0]
        # df2.to_excel(path)

        df3 = pd.DataFrame.from_dict(result)
        if os.path.isfile(fname):
            df3.to_csv(fname, mode = 'a', index = False, header = False)
        else:
            df3.to_csv(fname, mode = 'w', index = False, header = True)        
