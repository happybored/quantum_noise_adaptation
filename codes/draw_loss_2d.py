...
loss_noise_surface = loss_noise_all[aa][:,aa]
...
# print(loss_noise_surface)

ax = fig.add_subplot(132, projection='3d')
ax.plot_trisurf(x_input,y_input,z_input,cmap=cm.coolwarm)
ax.scatter(X,Y[5],Z1,cmap=cm.coolwarm)
ax.scatter(X[5],Y,Z2,cmap=cm.coolwarm)
...
loss_noise_surface = loss_diff_all[aa][:,aa]
...
# X = np.arange(-0.1,0.12,0.02)
# Y = np.arange(-0.1,0.12 ,0.02)
# loss_perfect_center = torch.load('loss_perfect_3.npy')
# loss_noise_center  = torch.load('loss_noise_3.npy')
# loss_diff_center  = torch.abs(loss_perfect_center- loss_noise_center)

# xx, yy = np.meshgrid(X, Y)
# newshape = (xx.shape[0])*(yy.shape[0])
# y_input = xx.reshape(newshape)
# x_input = yy.reshape(newshape)
# z_diff = loss_diff_all.reshape(newshape)
# ax = fig.add_subplot(236, projection='3d')
# ax.plot_trisurf(x_input,y_input,z_diff,cmap=cm.coolwarm)
