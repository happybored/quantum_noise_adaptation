...
#set noise model
    ...
    if args.static:
        ...
        # print(center_model)
    model = torch.load('database_real_dev_jakarta/{}/{}.pth'.format(args.pruning_type,args.modelpath))

    # test
    valid(dataflow, 'test', model, device, qiskit=False)


    print(f"\nTest with Qiskit Real Device")
    processor_simulation = QiskitProcessor(use_real_qc=True,backend_name=backend.name(),initial_layout=[0,1,2,3])
...
if __name__ == '__main__':
    ...
    sys.stdout = Logger('./logger/{}_log.txt'.format('real_qc'))
    import datetime    
    ##load csv 
    from qiskit import IBMQ
    ...
    for i in range(0,len(df2)):
        ...
        print()
        result = main(None,None,backend,df2['timestamp'].loc[i])
