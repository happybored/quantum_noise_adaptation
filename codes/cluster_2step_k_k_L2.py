...
#verify our metrix
df2['dis'] = np.linalg.norm(df2[keys],ord=2,axis=1)
corr = df2['dis'].corr(df2['accuracy'])
print(corr)

# define the first cluster
X = df2[keys]
clus = KMeansL1L2(4, norm='L2')
clus.fit(X)
...
for i in range(np.unique(label).size):
    ...
    samples['in_class_dis'] = np.linalg.norm(samples[keys]-mean_sample,ord=2,axis=1)
...
for cluster2 in clusters2:
    ...
    X = cluster2['df'][keys]
    clus = KMeansL1L2(clusters_num, norm='L2')
