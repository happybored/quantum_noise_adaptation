...
path ='calibration_data/cluster_res_iris_jakarta/ibmq_jakarta-tech-offline.xlsx'
online_path = 'calibration_data/cluster_res_iris_jakarta/ibmq_jakarta-tech-online.xlsx'
online_res_path = 'calibration_data/cluster_res_iris_jakarta/ibmq_jakarta-tech-online-dis.xlsx'
cluster_name = 'k_k_L1'
cluster_info_path = 'calibration_data/cluster_res_iris_jakarta/ibmq_jakarta-tech-offline-cluster-{}-info.xlsx'.format(cluster_name)
cluster_res_path = 'calibration_data/cluster_res_iris_jakarta/ibmq_jakarta-tech-offline-cluster-{}-result.xlsx'.format(cluster_name)


df = pd.read_excel(path)
df2 = df[keys]
df2['timestamp'] = df['timestamp']
df2['accuracy'] = df['training_first_day']
...
df_cluster = pd.DataFrame(clusters)
df_cluster = df_cluster.sort_values(by='num',ascending = False)
df_cluster = df_cluster.drop(df_cluster[df_cluster['num']<3].index)
print(df_cluster)
